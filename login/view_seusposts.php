...
<p class="w3-center" class="introducao"><em><b>Aqui estão os seus posts.<br> Continue colaborando talvez seus post seja o destaque da revista!! <br>Inspire-se e inspire novas possibilidades </b></em></p><br>

<h2  class="w3-center" class="introducao"> Destaques</h2>

<?php
ini_set('display_errors', 1);

$mysqli = new mysqli("localhost","root","1030", "revistaart");
$query = "SELECT p.idpost,p.nomePost, p.descricao,u.nome,u.idUsuario,tp.nomTag FROM post AS p join usuario AS u
ON p.idUsuario=u.idUsuario join tagpost AS tp  ON p.idTag=tp.idTag  ORDER BY p.idpost";
...
while (list($id, $nomePost, $descricao, $nome,$nomTag) = $result->fetch_row()) {
  ...
  <a class='btn btn-default btn-sm' title='Excluir' href='delete.php?id=$id'><i class='glyphicon glyphicon-trash'></i></a></td>";
    echo "<tr>";
    <a class='btn btn-default btn-sm' title='Editar' href='edit.php?id=$id'><i class='glyphicon glyphicon-edit'></i></a>
  <a class='btn btn-default btn-sm' title='Excluir' href='delete.php?id=$id'><i class='glyphicon glyphicon-trash'></i></a></td>";
    echo "<tr>";
}
